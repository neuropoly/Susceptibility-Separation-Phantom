function [FIT3D,Residuals,b]=PolyFitShimLike(Volumenii,Mask,order);
...
Indices=find(Mask.img(:,:,:));
[x1,y1,z1]=ind2sub(size(Volumenii.img(:,:,:)),Indices);
R=Volumenii.img(Indices);
% keyboard
...
%     for k=1:10,subplot(5,2,k),plot(model(:,k)),end
    ...
    clear model
    FIT3D=Volumenii;
    FIT3D.img=reshape(Fit,dim);
    Residuals=FIT3D;
    Residuals.img=(Volumenii.img-FIT3D.img);
else
    FIT3D=Volumenii;
